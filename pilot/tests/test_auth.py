...
async def test_websocket_auth_valid_token(mock_start_agent_session, mock_verify_id_token):
    ...
    try:
        with client.websocket_connect("/ws/test-session?is_audio=false", headers={"Sec-WebSocket-Protocol": "Bearer, valid-dummy-token"}) as websocket:
...
async def test_websocket_auth_invalid_token(mock_verify_id_token):
    ...
    with pytest.raises(WebSocketDisconnect) as excinfo:
        with client.websocket_connect("/ws/test-session?is_audio=false", headers={"Sec-WebSocket-Protocol": "Bearer, invalid-dummy-token"}) as websocket:
